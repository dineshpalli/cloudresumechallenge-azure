...
Thanks to [Jan F. Lammering](https://www.linkedin.com/in/jan-f-lammering/) and the amazing HR team of [Oraylis](https://www.linkedin.com/company/oraylis-gmbh/posts/?feedView=all) ([Sonja Kühle-BogenschneiderView Sonja Kühle-Bogenschneider’s profile ](https://www.linkedin.com/in/sonja-kühle-bogenschneider/), [Anne Müller-Kemler](https://www.linkedin.com/in/anne-müller-kemler-a43962222/), [Georgia Thume](https://www.linkedin.com/in/georgia-thume-883a59ab/))
 for inspiring me to expound my knowledge and skills with Azure and the related technologies.

Related Links:

My website - https://www.dineshpalli.com, please leave your valuable feedback in the comments section below, on my [LinkedIn Post](https://www.linkedin.com/posts/dineshpalli_cloudresumechallenge-microsoftazure-azuredevops-activity-7316066247262031872-_z5m?utm_source=share&utm_medium=member_desktop&rcm=ACoAABtfn7IBvP-I7xTwrT4Otmr20iqXbNykMfg).
...
I learned that **DNS is essentially the phonebook of the Internet** – it maps human-friendly names (like `myresume.com`) to the numeric IP addresses that computers use ([What is DNS? by Cloudflare](https://www.cloudflare.com/learning/dns/what-is-dns/#:~:text=What%20is%20DNS%3F)). When someone types my domain, a DNS query is made behind the scenes to find the matching IP or address for the server where my site is hosted. I also learned about **authoritative name servers** (which hold the DNS records for my domain) and **recursive resolvers** (the “DNS librarians” that fetch the answer) ([video](https://www.youtube.com/watch?v=mpQZVYPuDGU, [What is DNS? | How DNS works | Cloudflare](https://www.cloudflare.com/learning/dns/what-is-dns/#:~:text=There%20are%204%20DNS%20servers,involved%20in%20loading%20a%20webpage)), [What is DNS? by Cloudflare](https://www.cloudflare.com/learning/dns/what-is-dns/#:~:text=it%20serves%20as%20a%20reference,to%20the%20requested%20record%2C%20it)). This helped me appreciate why sometimes I’d get different results during propagation – DNS servers around the world take time to update.
...
Setting up GitHub Actions for Azure required creating a **Service Principal** – essentially a special Azure AD identity with permissions that GitHub Actions can use to deploy resources on my behalf. A service principal is an identity made for automated tools (like CI/CD pipelines) rather than a human user ([Service principals for CI/CD - Azure Databricks](https://learn.microsoft.com/en-us/azure/databricks/dev-tools/ci-cd/ci-cd-sp#:~:text=This%20article%20describes%20how%20to,automated%20tools%20and%20applications%2C%20including)). I created one using Azure CLI (`az ad sp create-for-rbac` command), giving it contributor access to my resource group. This generated an app ID, tenant ID, and client secret – which I stored as secrets in my GitHub repository. It felt a bit like giving keys to a robot: with those credentials, my GitHub Actions could log in to Azure and perform deployments securely.
...
[LinkedIn Post](https://www.linkedin.com/posts/dineshpalli_cloudresumechallenge-microsoftazure-azuredevops-activity-7316066247262031872-_z5m?utm_source=share&utm_medium=member_desktop&rcm=ACoAABtfn7IBvP-I7xTwrT4Otmr20iqXbNykMfg)
